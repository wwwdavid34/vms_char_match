...
#        run_one    = cfg['flow_control']['run_one']
        ignr_pend  = cfg['flow_control']['ignr_pend'] #do not process previous pending records
        skip_pred  = cfg['flow_control']['skip_pred'] #skip prediction to save time if already predicted
        skip_clas  = cfg['flow_control']['skip_clas'] #skip status classicitation
...
##        print 'length     :',length
        ...
        if end_date_dt>(start_date_dt+interval_dt):
            ...
            mid_date_dt =vms_tools.conv_str_to_dt('19000101') #an impossibly early date

            while mid_date_dt < end_date_dt:
                mid_date_dt = mid_date_dt_buf+interval_dt
                date_step = (vms_tools.conv_dt_to_vms_date(mid_date_dt_buf)[0:8],
...
#        print date_list
        date_list=[vms_tools.conv_dt_to_vms_date(start_date_dt),
...
#        for step in date_list:
        step=date_list
        self.update_all(step[0],step[1],gear_type,mode,
                        trans_no=trans_no,
    ...
    def update_all(self,start_date,end_date,gear_type,mode,
                   trans_no=None,vessel_name=None,
                   skip_pred=False,skip_clas=False,
                   ignr_pend=False):
        '''
...
#                print ini_idx_1,ini_idx_2
        ...
        for i in range(ini_idx,end_idx):
            ...
            print not ignr_pend
#            raw_input()
            self.update_selected(vessel[trans_no_idx],
                                 vessel[vessel_name_idx],
    ...
    def update_selected(self,trans_no,vessel_name,gear_type,start_date,end_date,
                        skip_pred=False,skip_clas=False,include_pending=True,skip_mtch=False):
        '''
...
##            self.update_predict(trans_no,vessel_name,gear_type,start_date,end_date)
        ...
        if not skip_clas:
            self.update_status(trans_no,vessel_name,gear_type,start_date,end_date,
                               proc=True,include_pending=include_pending)
#        sys.exit(1)
...
##            self.update_match(trans_no,vessel_name,gear_type,start_date,end_date)
    ...
    def get_vessel_list(self,gear_type=None):
        ...
        cur.execute('select * from vessel where registered_gear_type=%s',[gear_type])
        data=cur.fetchall()
    ...
    def update_predict(self,trans_no,vessel_name,gear_type,start_date,end_date):
        '''
        ...
        trk,hdr=vms_tools.get_vms_track(trans_no,
                                        vessel_name=vessel_name,
                                        gear_type=gear_type,
                                        start_date=start_date,
                                        end_date=end_date)
...
#        print 'prd',prd
        ...
        for t in trk:
            trk_list=list(t)
            trk_list.extend([vms_constants.null_val_str,
                             vms_constants.null_val_str])
            year_idx = hdr.index('REPORTDATE')
            year = ''.join(['y',str(trk_list[year_idx])[0:4]])
#            try:
            cur.execute(sql.SQL("insert into {} values ("+','.join(['%s' for i in trk_list])+") on conflict (id_key) do nothing").format(sql.Identifier(year)),[str(i) for i in trk_list])
            conn.commit()
#            except:
#                print 'Insert or ignore error:',trk_list
        ...
        for p in prd:
            prd_list=list(p)
            prd_list.extend([vms_constants.null_val_str,
                             vms_constants.null_val_str])
...
#            insert_val=[str[i] for i in prd_list]
            insert_val=prd_list[:]
            insert_val.extend([vms_constants.null_val_str,
                             vms_constants.null_val_str])
...
#            raw_input()
            cur.execute(sql.SQL('insert into {tbl} values ({val1}) on conflict (id_key) do update set {sta}={val_sta}, {loc}={val_loc}').format(tbl=sql.Identifier(year),sta=sql.Identifier('status'),loc=sql.Identifier('locale'),val1=sql.SQL(',').join(sql.Placeholder()* len(prd_list)),val_sta=sql.Placeholder(),val_loc=sql.Placeholder()),insert_val)
            conn.commit()
#            except:
#                print 'Insert or replace error:',prd_list
    ...
    def update_status(self,trans_no,vessel_name,gear_type,start_date,end_date,proc=False,include_pending=True):
        '''
        ...
        begin_date = vms_tools.get_begin_date(trans_no,
                                              vessel_name=vessel_name,
                                              gear_type=gear_type)
...
#        print site
        ...
        for i in range(0,len(trk)):
            ...
            if not tbuf is None and g != vms_constants.null_val_str:
                d1 = vms_tools.VMS_Time2Julian(tbuf[rpd_idx],format='%Y-%m-%d %H:%M:%S')
                d2 = vms_tools.VMS_Time2Julian(t[rpd_idx],format='%Y-%m-%d %H:%M:%S')
                dt = round(abs(d2-d1)*24.,2)
                lat1 = tbuf[lat_idx]
                lon1 = tbuf[lon_idx]
                lat2 = t[lat_idx]
                lon2 = t[lon_idx]
                d = vms_tools.getDist({'x':lon1,'y':lat1},
                                      {'x':lon2,'y':lat2})/1000.
                d = round(d,2)
                if dt==0:
                    v=vms_constants.null_val_str
                    ...
                    v = round(d/dt,2)
                    h1 = vms_tools.getHeading({'x':lon1,'y':lat1},
                                              {'x':lon2,'y':lat2})
                    if i+1 < len(trk):
                        tt = trk[i+1]
                        lat3 = tt[lat_idx]
                        lon3 = tt[lon_idx]
                        h2 = vms_tools.getHeading({'x':lon2,'y':lat2},
                                                  {'x':lon3,'y':lat3})
                        h = round(h2-h1,2)
                    else:
                        h = vms_constants.null_val_str
...
#            print s,g,l,v,h,d,dt
            ...
            cur.execute(sql.SQL('update {} set status=%s, segment=%s, locale=%s,velocity=%s,heading_change=%s,distance=%s,time_elapsed=%s where id_key=%s').format(sql.Identifier(year)),[s,g,l,v,h,d,dt,t[id_idx]])

            conn.commit()
        conn.close()

    def update_match(self,trans_no,vessel_name,gear_type,start_date,end_date,proc=False,include_pending=True):
        '''
        This module udpates the VBD match for PRD records.
        '''
        vms_proc_db=vms_ask_path.vms_proc_db_psql
        prdrec,vmshdr=vms_tools.vms_get_prd(trans_no,start_date,end_date,vms_proc_db,
                                         vessel_name=vessel_name,gear_type=gear_type)
        conn=psycopg2.connect(vms_proc_db)
        cur=conn.cursor()
        for rec in prdrec:
            ...
            vbd_rec,vbdhdr,dtime,dist=vms_tools.find_vbd(lat,lon,rpd,time_thr=10,dist_thr=1000)
#            print vbd_rec
#            raw_input()
...
#            raw_input()
            ...
            cur.execute(sql.SQL('update {tbl} set vbd_id_key=%s, vbd_dist=%s, vbd_dtime=%s where id_key=%s').format(tbl=tbl),[vbd_idk,round(dist,1),dtime,idk])
        conn.commit()
        conn.close()
