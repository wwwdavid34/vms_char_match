...
def query_by_latlon_kkp(lat_in,lon_in):
    '''
    Return landing site info dictionary by lat/lon input.
    Default searching radius is 0.1 degrees and nearest.
    '''
    lat = float(lat_in)
    lon = float(lon_in)
    landing_db = vms_ask_path.vms_proc_db_psql
    conn = psycopg2.connect(landing_db)
    cur = conn.cursor()
    cur.execute(sql.SQL('SELECT * FROM {tbl} WHERE ST_Within(%s,%s)').format(tbl=sql.Identifier('kkp_registered_landing_site')),[lat_in,lon_in])
    ldnst=cur.fetchall()
